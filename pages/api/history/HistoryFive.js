 
export default function HistoryFive() {  
	return (
		<div>
			<h3>2020년 34회 수상자 리스트</h3> 
			<div className="flex">
				<table className="table">
					<thead>
						<tr>
							<th colSpan="3">1일차 ｜ 디지털음원 부문</th>
						</tr>
						<tr> 
							<th></th> 
							<th>아티스트</th>
							<th>곡명</th>
						</tr>
					</thead>
					<tbody>
					<tr>
						<th scope="row">디지털음원 대상</th>
						<td>방탄소년단</td>
						<td>작은 것들을 위한 시</td>
					</tr>
					<tr>
						<th scope="row" rowSpan="10">디지털음원 본상</th>
						<td>방탄소년단</td>
						<td>작은 것들을 위한 시</td>
					</tr>
					<tr>
						<td>엠씨더맥스</td>
						<td>넘쳐흘러</td>
					</tr>
					<tr>
						<td>잔나비</td>
						<td>주저하는 연인들을 위해</td>
					</tr>
					<tr>
						<td>제니</td>
						<td>SOLO</td>
					</tr>
					<tr>
						<td>청하</td>
						<td>벌써 12시</td>
					</tr>
					<tr>
						<td>태연</td>
						<td>사계</td>
					</tr>
					<tr>
						<td>폴킴</td>
						<td>너를 만나</td>
					</tr>
					<tr>
						<td>AKMU</td>
						<td>어떻게 이별까지 사랑하겠어, 널 사랑하는 거지</td>
					</tr>
					<tr>
						<td>ITZY</td>
						<td>달라달라</td>
					</tr>
					<tr>
						<td>TWICE</td>
						<td>YES or YES</td>
					</tr>

					<tr>
						<th scope="row">제작자상</th>
						<td>방시혁</td>
						<td/>
					</tr>
					<tr>
						<th scope="row">베스트 R&B 힙합</th>
						<td>지코</td>
						<td/>
					</tr>
					<tr>
						<th scope="row">베스트 트로트</th>
						<td>송가인</td>
						<td/>
					</tr>
					<tr>
						<th scope="row">베스트 솔로 아티스트</th>
						<td>화사</td>
						<td/>
					</tr>
					<tr>
						<th scope="row">베스트 그룹</th>
						<td>마마무</td>
						<td/>
					</tr>
					<tr>
						<th scope="row" rowSpan="2">신인상</th>
						<td>ITZY</td>
						<td/>
					</tr>
					<tr>
						<td>TOMORROW X TOGEtdER</td>
						<td/>
					</tr>
					<tr>
						<th scope="row" rowSpan="3">넥스트 제너레이션</th>
						<td>김재환</td>
						<td/>
					</tr>
					<tr>
						<td>AB6IX</td>
						<td/>
					</tr>
					<tr>
						<td>ATEEZ</td>
						<td/>
					</tr>
					</tbody>
				</table> 
				<table className="table">
					<thead>
						<tr>
							<th colSpan="3">2일차 ｜ 음반 부문</th>
						</tr>
						<tr> 
							<th></th> 
							<th>아티스트</th>
							<th>곡명</th>
						</tr>
					</thead>
					<tbody>
					<tr>
						<th scope="row">음반 대상</th>
						<td>방탄소년단</td>
						<td>MAP OF tdE SOUL : PERSONA</td>
					</tr>
					<tr>
						<th scope="row" rowSpan="10">음반 본상</th>
						<td>뉴이스트</td>
						<td>tde 6td Mini Album &rsquo;Happily Ever After&rsquo;</td>
					</tr>
					<tr>
						<td>방탄소년단</td>
						<td>MAP OF tdE SOUL : PERSONA</td>
					</tr>
					<tr>
						<td>백현</td>
						<td>City Lights - tde 1st Mini Album</td>
					</tr>
					<tr>
						<td>세븐틴</td>
						<td>3RD ALBUM &rsquo;An Ode&rsquo;</td>
					</tr>
					<tr>
						<td>세훈&찬열</td>
						<td>What a life - tde 1st Mini Album</td>
					</tr>
					<tr>
						<td>슈퍼주니어</td>
						<td>Time_Slip - tde 9td Album</td>
					</tr>
					<tr>
						<td>GOT7</td>
						<td>SPINNING TOP : BETWEEN SECURITY &amp; INSECURITY</td>
					</tr>
					<tr>
						<td>몬스타엑스</td>
						<td>WE ARE HERE - tde 2nd Album Take.2</td>
					</tr>
					<tr>
						<td>NCT DREAM</td>
						<td>We Boom - tde 3rd Mini Album</td>
					</tr>
					<tr>
						<td>TWICE</td>
						<td>Feel Special</td>
					</tr>

					<tr>
						<th scope="row">틱톡 골든디스크 인기상</th>
						<td>방탄소년단</td>
						<td/>
					</tr>
					<tr>
						<th scope="row">왕이윈뮤직 팬스 초이스 케이팝 스타상</th>
						<td>방탄소년단</td>
						<td/>
					</tr>
					<tr>
						<th scope="row" rowSpan="2"/>
						<td>뉴이스트</td>
						<td/>
					</tr>
					<tr>
						<td>TWICE</td>
						<td/>
					</tr>

					<tr>
						<th scope="row">베스트 퍼포먼스(남)</th>
						<td>아스트로</td>
						<td/>
					</tr>
					<tr>
						<th scope="row">베스트 퍼포먼스(여)</th>
						<td>(여자)아이들</td>
						<td/>
					</tr>
					<tr>
						<th scope="row">베스트 O.S.T.</th>
						<td>거미</td>
						<td/>
					</tr>
					</tbody>
				</table>  
			</div>

			<h3>2021년 35회 수상자 리스트</h3> 
			<div className="flex">
			 	<table className="table">
					<thead>
						<tr>
							<th colSpan="3">1일차 ｜ 디지털음원 부문</th>
						</tr>
						<tr> 
							<th></th> 
							<th>아티스트</th>
							<th>곡명</th>
						</tr>
					</thead>
				<tbody>
				<tr>
					<th scope="row">디지털음원 대상</th>
					<td>아이유</td>
					<td>Blueming</td>
				</tr>
				<tr>
					<th scope="row" rowSpan="10">디지털음원 본상</th>
					<td>BLACKPINK</td>
					<td>How You Like tdat</td>
				</tr>
				<tr>
					<td>ITZY</td>
					<td>WANNABE</td>
				</tr>
				<tr>
					<td>노을</td>
					<td>늦은 밤 너의 집 앞 골목에서</td>
				</tr>
				<tr>
					<td>레드벨벳</td>
					<td>Psycho</td>
				</tr>
				<tr>
					<td>마마무</td>
					<td>Hip</td>
				</tr>
				<tr>
					<td>방탄소년단</td>
					<td>Dynamite</td>
				</tr>
				<tr>
					<td>아이유</td>
					<td>Blueming</td>
				</tr>
				<tr>
					<td>오마이걸</td>
					<td>살짝 설렜어(Nonstop)</td>
				</tr>
				<tr>
					<td>지코</td>
					<td>아무노래</td>
				</tr>
				<tr>
					<td>화사</td>
					<td>마리아(Maria)</td>
				</tr>

				<tr>
					<th scope="row">베스트 발라드</th>
					<td>이승기</td>
					<td/>
				</tr>
				<tr>
					<th scope="row">베스트 트로트</th>
					<td>임영웅</td>
					<td/>
				</tr>
				<tr>
					<th scope="row">베스트 R&B 힙합</th>
					<td>창모</td>
					<td/>
				</tr>
				<tr>
					<th scope="row">베스트 솔로 아티스트</th>
					<td>제시</td>
					<td/>
				</tr>
				<tr>
					<th scope="row">베스트 그룹</th>
					<td>몬스타엑스</td>
					<td/>
				</tr>
				<tr>
					<th scope="row">Trend of tde year</th>
					<td>지코</td>
					<td/>
				</tr>
				<tr>
					<th scope="row">골든 초이스</th>
					<td>뉴이스트</td>
					<td/>
				</tr>
				<tr>
					<th scope="row" rowSpan="2">넥스트 제너레이션</th>
					<td>더보이즈</td>
					<td/>
				</tr>
				<tr>
					<td>이달의 소녀</td>
					<td/>
				</tr>
				</tbody>
				</table> 
				<table className="table">
					<thead>
					<tr>
							<th colSpan="3">2일차 ｜ 음반 부문</th>
						</tr>
						<tr> 
							<th></th> 
							<th>아티스트</th>
							<th>곡명</th>
						</tr>
					</thead>
				<tbody>
				<tr>
					<th scope="row">음반 대상</th>
					<td>방탄소년단</td>
					<td>MAP OF tdE SOUL : 7</td>
				</tr>
				<tr>
					<th scope="row" rowSpan="10">음반 본상</th>
					<td>BLACKPINK</td>
					<td>tdE ALBUM</td>
				</tr>
				<tr>
					<td>EXO</td>
					<td>OBSESSION</td>
				</tr>
				<tr>
					<td>GOT7</td>
					<td>DYE</td>
				</tr>
				<tr>
					<td>NCT</td>
					<td>NCT-tde 2nd Albuem RESONANCE Pt.1</td>
				</tr>
				<tr>
					<td>NCT 127</td>
					<td>NCT #127 Neo Zone</td>
				</tr>
				<tr>
					<td>TWICE</td>
					<td>MORE & MORE</td>
				</tr>
				<tr>
					<td>방탄소년단</td>
					<td>MAP OF tdE SOUL : 7</td>
				</tr>
				<tr>
					<td>백현</td>
					<td>Delight</td>
				</tr>
				<tr>
					<td>세븐틴</td>
					<td>헹가래</td>
				</tr>
				<tr>
					<td>투모로우바이투게더</td>
					<td>꿈의 장: ETERNITY</td>
				</tr>

				<tr>
					<th scope="row">큐라프록스 골든디스크 인기상</th>
					<td>방탄소년단</td>
					<td/>
				</tr>
				<tr>
					<th scope="row">큐큐뮤직 팬스 초이스 케이팝 스타상</th>
					<td>EXO</td>
					<td/>
				</tr>
				<tr>
					<th scope="row">코스모폴리탄 아티스트</th>
					<td>NCT 127</td>
					<td/>
				</tr>
				<tr>
					<th scope="row">베스트 퍼포먼스(남)</th>
					<td>스트레이키즈</td>
					<td/>
				</tr>
				<tr>
					<th scope="row">베스트 퍼포먼스(여)</th>
					<td>(여자)아이들</td>
					<td/>
				</tr>
				<tr>
					<th scope="row">베스트 O.S.T.</th>
					<td>조정석</td>
					<td/>
				</tr>
				<tr>
					<th scope="row" rowSpan="3">신인상</th>
					<td>ENHYPEN</td>
					<td/>
				</tr>
				<tr>
					<td>TREASURE</td>
					<td/>
				</tr>
				<tr>
					<td>김호중</td>
					<td/>
				</tr>
				</tbody>
				</table> 
			</div>

			<h3>2022년 36회 수상자 리스트</h3>
			<div className="flex">
				<table className="table">
					<thead>
						<tr>
							<th colSpan="3">1일차 ｜ 디지털음원 부문</th>
						</tr>
						<tr> 
							<th></th> 
							<th>아티스트</th>
							<th>곡명</th>
						</tr>
					</thead>
					<tbody>
						<tr>
							<th scope="row">디지털음원 대상</th>
							<td>아이유</td>
							<td>Celebrity</td>
						</tr>
						<tr>
							<th scope="row" rowSpan="8">
								디지털음원 본상
							</th>
							<td>aespa</td>
							<td>Next Level</td>
						</tr>
						<tr>
							<td>AKMU</td>
							<td>낙하(witd 아이유)</td>
						</tr>
						<tr>
							<td>STAYC</td>
							<td>ASAP</td>
						</tr>
						<tr>
							<td>방탄소년단</td>
							<td>Butter</td>
						</tr>
						<tr>
							<td>아이유</td>
							<td>Celebrity</td>
						</tr>
						<tr>
							<td>오마이걸</td>
							<td>Dun Dun Dance</td>
						</tr>
						<tr>
							<td>이무진</td>
							<td>신호등</td>
						</tr>
						<tr>
							<td>헤이즈</td>
							<td>헤픈 우연</td>
						</tr>

						<tr>
							<th scope="row" rowSpan={2}>
								신인상
							</th>
							<td>aespa</td>
							<td />
						</tr>
						<tr>
							<td>STAYC</td>
							<td />
						</tr>
						<tr>
							<th scope="row">베스트 그룹</th>
							<td>브레이브걸스</td>
							<td />
						</tr>
						<tr>
							<th scope="row">베스트 솔로 아티스트</th>
							<td>임영웅</td>
							<td />
						</tr>
						<tr>
							<th scope="row" rowSpan={2}>
								베스트 퍼포먼스
							</th>
							<td>더보이즈</td>
							<td />
						</tr>
						<tr>
							<td>전소미</td>
							<td />
						</tr>
					</tbody>
				</table> 
				<table className="table">
					<thead>
						<tr>
							<th colSpan="3">2일차 ｜ 음반 부문</th>
						</tr>
						<tr> 
							<th></th> 
							<th>아티스트</th>
							<th>곡명</th>
						</tr>
					</thead>
				<tbody>
					<tr>
						<th scope="row">음반 대상</th>
						<td>방탄소년단</td>
						<td>BE</td>
					</tr>
					<tr>
						<th scope="row" rowSpan="8">
							음반 본상
						</th>
						<td>ENHYPEN</td>
						<td>DIMENSION : DILEMMA</td>
					</tr>
					<tr>
						<td>NCT127</td>
						<td>Sticker - tde 3rd Album</td>
					</tr>
					<tr>
						<td>NCT DREAM</td>
						<td>맛 (Hot Sauce) - tde 1st Album</td>
					</tr>
					<tr>
						<td>Stray Kids</td>
						<td>NOEASY</td>
					</tr>
					<tr>
						<td>방탄소년단</td>
						<td>BE</td>
					</tr>
					<tr>
						<td>세븐틴</td>
						<td>9td Mini Album &rsquo;Attacca&rsquo;</td>
					</tr>
					<tr>
						<td>아이유</td>
						<td>5td Album &rsquo;LILAC&rsquo;</td>
					</tr>
					<tr>
						<td>투모로우바이투게더</td>
						<td>혼돈의 장 : FREEZE</td>
					</tr>

					<tr>
						<th scope="row" rowSpan={2}>
							코스모폴리탄
							<br />
							아티스트상
						</th>
						<td>세븐틴</td>
						<td />
					</tr>
					<tr>
						<td>aespa</td>
						<td></td>
					</tr>
					<tr>
						<th scope="row">
							ARTIST OF tdE<br />YEAR
						</th>
						<td>aespa</td>
						<td></td>
					</tr>
					<tr>
						<th scope="row" rowSpan={2}>
							seezn 골든디스크
							<br />
							인기상
						</th>
						<td>방탄소년단</td>
						<td></td>
					</tr>
					<tr></tr>
				</tbody>
				</table> 
			</div>
		</div>
	)
} 
	
	
	
	
	